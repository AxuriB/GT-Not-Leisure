        ...
        private boolean work(ItemStack item, EntityPlayerMP player, ItemStack exItem, int slot, int x, int y, int z) {
            ...
            if (!obj.rangeCheck()) {
                ...
                if (securityCheck(player, grid, SecurityPermissions.EXTRACT)) {
                    ...
                    var aeItem = iItemStorageChannel.extractItems(AEItemStack.create(exItem).setStackSize(exItem.stackSize), Actionable.SIMULATE, new PlayerSource(player, obj));
                    if (aeItem != null && aeItem.getStackSize() > 0) {
                        var aeitem = iItemStorageChannel.extractItems(AEItemStack.create(exItem).setStackSize(aeItem.getStackSize()), Actionable.MODULATE, new PlayerSource(player, obj));

                        player.inventory.setInventorySlotContents(slot, aeitem.setStackSize(aeitem.getStackSize() + handItemConnt).getItemStack());
                        return true;
                    }
                }
            }
