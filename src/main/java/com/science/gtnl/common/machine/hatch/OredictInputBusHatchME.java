...
public class OredictInputBusHatchME extends MTEHatchInputBusME {

    @Nullable
    private String oreDict;

    public OredictInputBusHatchME(int aID, String aName, String aNameRegional) {
        super(aID, true, aName, aNameRegional);
    }

    public OredictInputBusHatchME(String aName, boolean autoPullAvailable, int aTier, String[] aDescription,
        ITexture[][][] aTextures) {
        ...
        if (gridProxy == null) {
            if (getBaseMetaTileEntity() instanceof IGridProxyable gridProxyable) {
                gridProxy = new AENetworkProxy(
                    gridProxyable,
                    "proxy",
                    GTNLItemList.OredictInputBusHatchME.get(1),
                    true);
    ...
    protected ModularWindow createStackSizeConfigurationWindow(EntityPlayer player) {
        ...
        builder.widget(
            TextWidget.localised("Info_OredictInputBusHatchME_Oredict")
                .setPos(3, 120)
                .setSize(50, 14))
            .widget(
    ...
    public String[] getInfoData() {
        String busStatusKey = getProxy() != null && getProxy().isActive()
            ? StatCollector.translateToLocal("Info_OredictInputBusHatchME_Online")
            : StatCollector.translateToLocalFormatted("Info_OredictInputBusHatchME_Offline", getAEDiagnostics());
        String oreDictStatusKey = hasOreDict()
            ? StatCollector.translateToLocalFormatted("Info_OredictInputBusHatchME_Oredict_Set", oreDict)
            : StatCollector.translateToLocal("Info_OredictInputBusHatchME_Oredict_Unset");
        return new String[] { busStatusKey,
            StatCollector.translateToLocal("Info_OredictInputBusHatchME_Oredict") + oreDictStatusKey };
    }
